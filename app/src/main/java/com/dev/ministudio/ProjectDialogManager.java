...
public class ProjectDialogManager {
    ...
    public void showRenameDialog(File targetFile, FileNode targetNode) {
        ...
        btnCancel.setOnClickListener(v -> inputDialog.dismiss());
        // ในไฟล์ ProjectDialogManager.java ภายในเมธอด showRenameDialog
...
btnConfirm.setOnClickListener(v -> {
    ...
    if (!newName.isEmpty() && !newName.equals(targetFile.getName())) {
        ...
        if (success) {
            ...
            // --- ใส่โค้ด 2 บรรทัดนี้ตรงนี้ครับ ---
            File renamedFile = new File(targetFile.getParent(), newName);
            ((MainActivity) context).onFileRenamed(targetFile, renamedFile);
            // ----------------------------------
