...
import android.util.TypedValue; // 🟢 เพิ่มคลาสสำหรับคำนวณหน่วยหน่วยฟอนต์ sp
...
public class MainActivity extends AppCompatActivity {
    ...
    // 🟢 ตัวแปรสำหรับคุมระบบย่อ-ขยาย ขนาดตัวอักษรหน้าจอ (หน่วยเป็น sp)
    ...
    private void setupShortcutBar(LinearLayout bar) {
        ...
        // 🟢 เพิ่มสัญลักษณ์ด่วนย่อ/ขยายฟอนต์ [ - ] และ [ + ] ไว้ท้ายสุดของระบบคีย์บอร์ดลัด
        ...
        // 🟢 ดีไซน์พื้นหลังปุ่มลัดย่อขยายหน้าจอ (สีแดงส้มพรีเมียม สังเกตง่าย)
        ...
        for (final String s : symbols) {
            ...
            if (s.equals("Enter")) {
                ...
                btn.setTextColor(android.graphics.Color.WHITE);
                btn.setTextSize(16); // ปรับเครื่องหมายให้ชัดเจนขึ้น
            } else {
            ...
            btn.setOnClickListener(v -> {
                if (s.equals(" - ")) {
                    // 🟢 ตรรกะการย่อขนาดฟอนต์ (จำกัดขั้นต่ำไม่ให้เล็กกว่า 10sp เพื่อไม่ให้ปวดตา)
                    if (currentCodeFontSize > 10.0f) {
                        currentCodeFontSize -= 1.5f;
                        applyEditorFontSize(currentCodeFontSize);
                    }
                } else if (s.equals(" + ")) {
                    // 🟢 ตรรกะการขยายขนาดฟอนต์ (จำกัดสูงสุดที่ 30sp เพื่อไม่ให้ทะลุขอบ)
                    if (currentCodeFontSize < 30.0f) {
                        currentCodeFontSize += 1.5f;
                        applyEditorFontSize(currentCodeFontSize);
                    }
                } else {
    ...
    // 🟢 ฟังก์ชันส่งคำสั่งควบคุมขนาดฟอนต์ไปยังกล่องรับโค้ดและแถบตัวเลขพร้อมๆ กัน
