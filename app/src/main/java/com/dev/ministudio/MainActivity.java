...
public class MainActivity extends AppCompatActivity {
    ...
    // 🟢 ตัวแปรสำหรับคุมระบบย่อ-ขยาย ขนาดตัวอักษรหน้าจอ (เริ่มต้น 14sp)
...
public ProjectModel getCurrentProject() {
    ...
    // แยกส่วน Editor ออกไปให้ EditorManager จัดการ
    new EditorManager(this, codeEditor, tvSaveStatus, lineNumbers);

    dialogManager = new ProjectDialogManager(this, this::triggerTreeRefresh);
...
private void openFile(File file) {
    ...
    if (name.endsWith(".png") || name.endsWith(".jpg") || name.endsWith(".jpeg") || name.endsWith(".webp") || name.endsWith(".gif")) {
        ...
        try {
            ...
            // 🟢 1. แจ้ง EditorManager ว่ากำลังโหลดไฟล์ (ป้องกัน TextWatcher ทำงานรัวๆ)
            if (editorManager != null) {
                editorManager.setProgrammaticChange(true);
            }
            
            codeEditor.setText(sb.toString());
            
            // 🟢 2. โหลดเสร็จแล้วค่อยปลดล็อก
            if (editorManager != null) {
                editorManager.setProgrammaticChange(false);
            }
            
            // 🟢 3. สั่ง Highlight ด้วยตัวเอง 1 ครั้งหลังจากโหลดเสร็จ
            ...
            e.printStackTrace();
            showToast("Read Error: " + e.getMessage());
        }
    }
}


// --- ส่วนที่ 2: เพิ่มเมธอดสำหรับรองรับระบบ Rename ---
public void onFileRenamed(File oldFile, File newFile) {
    // รีเฟรชรายการไฟล์ทั้งหมดใน TreeView หลังเปลี่ยนชื่อ
    refreshFileTree(); 
    
    // อัปเดตสถานะไฟล์ที่เปิดค้างไว้ (ถ้ามี)
    if (currentProject != null && currentProject.getCurrentOpenFile() != null && 
        currentProject.getCurrentOpenFile().equals(oldFile)) {
        currentProject.setCurrentOpenFile(newFile);
        updateFilePathStatus(newFile);
    }
}
    ...
    private void setupShortcutBar(LinearLayout bar) {
        ...
        // แป้นคีย์บอร์ดลัดแนวนอน พร้อมสัญลักษณ์ด่วนย่อ/ขยายฟอนต์ [ - ] และ [ + ] ไว้ท้ายสุด
        ...
        // ดีไซน์สีปุ่ม Zoom สีแดงพรีเมียม สังเกตง่าย
        ...
        for (final String s : symbols) {
            ...
            btn.setOnClickListener(v -> {
                if (s.equals(" - ")) {
                    // 🟢 ตรรกะการย่อขนาดฟอนต์ (จำกัดขั้นต่ำไม่ให้เล็กกว่า 10sp เพื่อถนอมสายตา)
                    if (currentCodeFontSize > 10.0f) {
                        currentCodeFontSize -= 1.5f;
                        applyEditorFontSize(currentCodeFontSize);
                    }
                } else if (s.equals(" + ")) {
                    // 🟢 ตรรกะการขยายขนาดฟอนต์ (จำกัดสูงสุดล็อกตามคำขอที่ 30.0fsp เพื่อประหยัดพื้นที่คีย์บอร์ด)
                    if (currentCodeFontSize < 20.0f) {
                        currentCodeFontSize += 1.5f;
                        applyEditorFontSize(currentCodeFontSize);
                    }
                } else {
    ...
    // 🟢 ส่งค่าสั่งงานความสูงฟอนต์ของทั้งเอดิเตอร์โค้ด และแถบเลขบรรทัดให้สัมพันธ์กัน
    private void applyEditorFontSize(float sizeSp) {
        if (codeEditor != null && lineNumbers != null) {
            codeEditor.setTextSize(TypedValue.COMPLEX_UNIT_SP, sizeSp);
            lineNumbers.setTextSize(TypedValue.COMPLEX_UNIT_SP, sizeSp);
