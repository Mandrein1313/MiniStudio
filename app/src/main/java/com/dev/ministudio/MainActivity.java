...
public class MainActivity extends AppCompatActivity {
    ...
    // 🐙 เมทอดรันบิวด์บนคลาวด์: ฟอร์แมตสไตล์ GitHub Actions + ภาษาไทย + สั่งหยุดอัตโนมัติ + ดึงชื่อไฟล์และบรรทัดที่เกิด Error
    private void startCloudBuildPipeline() {
        ...
        // ตัวแปรสถานะภายใน Listener สำหรับสั่งตัดวงจรทันทีเมื่อเจอจุดพัง
        final boolean[] isAbortedDueToError = {false};

        // 🟢 เริ่มต้นจำลองกลุ่มคำสั่งตามสไตล์ GitHub Setup (ภาษาไทย)
        appendLog("##[group]เริ่มขั้นตอนการตั้งค่า & ตรวจสอบโปรเจกต์เบื้องต้น", Color.parseColor("#8B949E")); // สีเทา GitHub Style
        appendLog("🔔 [กำลังจัดเตรียมสภาพแวดล้อม...] เริ่มทำงานระบบ Workflow สำเร็จ", Color.parseColor("#C9D1D9"));
        appendLog("📂 ที่อยู่โปรเจกต์ (Root Path): " + currentProject.getRootPath(), Color.parseColor("#58A6FF")); // สีฟ้า GitHub Info
        ...
        BuildTaskManager buildTask = new BuildTaskManager(
            ...
            new BuildTaskManager.BuildListener() {
                ...
                public void onLogAppend(String text, int color) { 
                    // 🛑 ตัดวงจร: ถ้าสั่ง Abort ไปแล้ว จะไม่ยอมรับเนื้อหา Log ใดๆ เพิ่มเติมอีกเด็ดขาด
                    if (isAbortedDueToError[0]) return;

                    String lowerText = text.toLowerCase();

                    // 🚨 สกัดจับและแปลงโหมดให้เป็น GitHub Error ทันทีหากตรวจพบคำสำคัญ
                    if (lowerText.contains("error:") || 
                        lowerText.contains("failed") || 
                        lowerText.contains("exception") || 
                        color == Color.RED) {
                        
                        isAbortedDueToError[0] = true; // ดึงคัตเอาท์หยุดการทำงานระบบบิวด์ทันที!
                        
                        // 🔍 [ฟีเจอร์เพิ่มใหม่] ระบบวิเคราะห์และดึงชื่อไฟล์ + บรรทัดที่พังจาก Log ของ Java Compiler
                        String errorFileDetails = "ไม่สามารถระบุไฟล์ได้แน่ชัด (โปรดดูที่ Log ด้านบน)";
                        try {
                            // ค้นหารูปแบบพาร์ทไฟล์ เช่น /.../MainActivity.java:234:
                            if (text.contains(".java:")) {
                                int javaIndex = text.indexOf(".java");
                                // ตัดเอาข้อความรอบๆ ชิ้นส่วนที่เป็นชื่อไฟล์มาวิเคราะห์
                                int startPos = text.lastIndexOf("/", javaIndex);
                                if (startPos == -1) startPos = text.lastIndexOf("\\", javaIndex);
                                if (startPos == -1) startPos = 0;
                                else startPos += 1; // ขยับข้ามเครื่องหมาย /
                                
                                // หาตำแหน่งจุดสิ้นสุดของเลขบรรทัด (เครื่องหมาย : ตัวถัดไปหลังชื่อไฟล์)
                                int endPos = text.indexOf(":", javaIndex + 5);
                                if (endPos != -1) {
                                    // ดึงข้อความออกมา เช่น MainActivity.java:234
                                    String rawFileAndLine = text.substring(startPos, endPos);
                                    String[] parts = rawFileAndLine.split(":");
                                    if (parts.length >= 2) {
                                        errorFileDetails = "📄 ไฟล์: " + parts[0] + " 📍 บรรทัดที่: " + parts[1];
                                    } else {
                                        errorFileDetails = "📄 ไฟล์: " + rawFileAndLine;
                                    }
                                }
                            }
                        } catch (Exception e) {
                            // หากเกิดข้อผิดพลาดในการตัดสตริง ให้ใช้ข้อความดั้งเดิมปลอดภัยที่สุด
                            errorFileDetails = "ไม่สามารถระบุไฟล์ได้แน่ชัด (โปรดดูที่ Log ด้านบน)";
                        }
                        
                        // 🐙 ฟอร์แมตแต่งข้อความ Error ให้เหมือนหลุดมาจาก GitHub Actions ใน Terminal (ภาษาไทย)
                        appendLog("\n##[group]❌ รายละเอียดข้อผิดพลาดในการคอมไพล์ซอร์สโค้ด", Color.parseColor("#FF7B72")); 
                        appendLog("##[error] กระบวนการหยุดทำงานด้วย Exit Code 1 (ตรวจพบปัญหาในโค้ดของคุณ)", Color.parseColor("#FF7B72")); // สีแดง GitHub Error
                        appendLog("##[error] จุดที่พัง -> " + errorFileDetails, Color.parseColor("#FF8A80")); // แสดงชื่อไฟล์และบรรทัดแบบเด่นชัด 🌟
                        appendLog("##[error] ข้อความจากระบบ: " + text.trim(), Color.parseColor("#FFA198")); // ไฮไลท์บรรทัดที่พังดั้งเดิม
                        appendLog("##[endgroup]", Color.parseColor("#FF7B72"));

                        // พ่นสรุปสถานะ Workflow แบบภาพรวมท้ายสุดของหน้าจอ
                        appendLog("\n------------------------------------------------------------------------", Color.parseColor("#8B949E"));
                        appendLog("❌ ข้อผิดพลาด: การทำงานล้มเหลว (Process completed with exit code 1)", Color.parseColor("#FF7B72"));
                        appendLog("💡 คำแนะนำ: โปรดแก้ไขจุดผิดพลาดใน " + errorFileDetails + " จากนั้นทำการตรวจสอบไวยากรณ์โค้ด หรือเครื่องหมายเซมิโคลอน (;) ก่อนกดรันใหม่อีกครั้ง", Color.parseColor("#F2CC60")); // สีเหลืองทองเตือนสติ
                        appendLog("------------------------------------------------------------------------", Color.parseColor("#8B949E"));
                        
                        showToast("💥 บิวด์ล้มเหลว! (Exit Code 1)");
                        return;
                    }

                    // หากไม่มี Error ให้พ่นเนื้อหาล็อกปกติออกมา (ใช้คู่สีโทนมืดของ GitHub เพื่อความสมจริง)
                    if (color == Color.GREEN) {
                        appendLog(text, Color.parseColor("#56D364")); // สีเขียวสว่าง GitHub
                    } else if (color == Color.YELLOW) {
                        appendLog(text, Color.parseColor("#E3B341")); // สีเหลืองสว่าง GitHub
                    } else if (color == Color.CYAN) {
                        appendLog(text, Color.parseColor("#3FB950")); // สีฟ้าอมเขียวสเตตัสรันสำเร็จ
                    } else {
                        appendLog(text, Color.parseColor("#C9D1D9")); // สีขาวนวล (Default Text ของ GitHub)
                    }
                }
                ...
                public void onBuildFinished(boolean success, String apkPath) {
                    // หากหน้าจอถูกดักสกัดพังไปก่อนหน้านี้แล้ว เมทอดนี้จะไม่ทำอะไรเพิ่มเติมเพื่อป้องกันข้อความทับซ้อน
                    if (isAbortedDueToError[0]) return;
                    ...
                    if (success) {
                        ...
                        appendLog("\n##[group]🎉 งานหลังบิวด์: จัดเก็บไฟล์ระบบแอปพลิเคชัน", Color.parseColor("#56D364"));
                        appendLog("✅ สำเร็จ: กระบวนการทำงานทั้งหมดเสร็จสิ้นโดยไม่มีข้อผิดพลาด", Color.parseColor("#56D364"));
                        appendLog("📦 ไฟล์แอปที่ได้ (APK): " + (apkPath != null ? apkPath : "outputs/apk/debug/app-debug.apk"), Color.parseColor("#58A6FF"));
                        appendLog("##[endgroup]", Color.parseColor("#56D364"));
                    } else {
                        showToast("กระบวนการทำงานล้มเหลว");
                        appendLog("\n##[error] การทำงานหยุดชะงักเนื่องจากการปิดตัวของระบบบิวด์อย่างกะทันหัน", Color.parseColor("#FF7B72"));
                    }
                }
            }
        );
