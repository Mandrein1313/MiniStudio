...
public class MainActivity extends AppCompatActivity {
    ...
    private void initViews() {
        ...
        // 🟢 จุดแก้ไขที่ 1: เรียกใช้เมธอดแบบไม่มีอาร์กิวเมนต์ให้ตรงกับการประกาศด้านล่าง
        setupShortcutBar();
    }
    ...
    private void setupShortcutBar() { 
        ...
        // 🟢 เปลี่ยนจาก "A+" และ "A-" เป็นไอคอนสัญลักษณ์ "↩" (Undo) และ "↪" (Redo) ให้เหมือนภาพ 95286.jpg
        String[] shortcuts = {
            "↩", "↪", "{", "}", "[", "]", "(", ")", "<", ">", 
            ";", ",", ".", "=", "+", "-", "*", "/", 
            "_", "\"", "'", ":", "?", "!", "|", "&"
        ...
        for (final String shortcut : shortcuts) {
            ...
            btn.setTextSize(18); // ปรับขนาดเพิ่มขึ้นเล็กน้อยเป็น 18sp เพื่อให้ไอคอนลูกศรดูเด่นชัดเจน
            btn.setGravity(Gravity.CENTER);
            btn.setPadding(paddingPx, 0, paddingPx, 0);
            btn.setTypeface(Typeface.MONOSPACE, Typeface.BOLD);
            
            // ปรับสีไอคอนควบคุมย้อนกลับ/ทำซ้ำให้เป็นสีขาวสว่างเด่นชัดแบบในตัวอย่าง
            ...
            // 🟢 อัปเดตเงื่อนไขตรรกะการตรวจสอบให้ตรงกับไอคอนใหม่
            btn.setOnClickListener(v -> {
                if (codeEditor == null) return;

                if (shortcut.equals("↩")) {
                    // ↩️ สั่ง Undo (ย้อนกลับการกระทำ) เมื่อกดไอคอนลูกศรซ้าย
                    if (codeEditor.canUndo()) {
                        codeEditor.undo();
                    }
                } else if (shortcut.equals("↪")) {
                    // ↪️ สั่ง Redo (ทำซ้ำการกระทำ) เมื่อกดไอคอนลูกศรขวา
                    if (codeEditor.canRedo()) {
                        codeEditor.redo();
                    }
                } else {
                    // แทรกตัวอักษรสัญลักษณ์ปกติลงในเอดิเตอร์
