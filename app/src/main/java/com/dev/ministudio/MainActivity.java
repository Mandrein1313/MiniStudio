...
public class MainActivity extends AppCompatActivity {
    ...
    // 🐙 เมทอดรันบิวด์บนคลาวด์: เวอร์ชันเพิ่มสีสันให้บล็อก Error สะดุดตา แยกแยะง่ายสไตล์ ANSI Terminal
            ...
            new BuildTaskManager.BuildListener() {
                ...
                public void onLogAppend(String text, int color) { 
                    ...
                    if (lowerText.contains("error:") || 
                        ...
                        // ระบบวิเคราะห์และดึงชื่อไฟล์ + บรรทัดที่พังจาก Log ของ Java Compiler
                        String fileName = "ไม่ระบุไฟล์";
                        String lineNumber = "0";
                        ...
                        // 🐙 [ปรับปรุงสีสันสดใส] ฟอร์แมตแต่งข้อความ Error ให้เด่นชัดแยกแยะง่ายด้วยคู่สีคู่ตรงข้าม
                        appendLog("\n##[group]❌ รายละเอียดข้อผิดพลาดในการคอมไพล์ซอร์สโค้ด", Color.parseColor("#FF453A")); // แดงสว่างระดับระบบปฏิบัติการ
                        appendLog("##[error] STATUS  -> กระบวนการหยุดทำงานด้วย Exit Code 1", Color.parseColor("#FF7B72")); 
                        
                        if (parseSuccess) {
                            // ไฮไลท์ข้อมูลไฟล์ผิดพลาดด้วยสีเหลืองทองและสีส้มสว่างเพื่อให้ผู้พัฒนาเพ่งเล็งจุดนี้เป็นพิเศษ 🌟
                            appendLog("##[error] TARGET  -> 📄 ไฟล์: ", Color.parseColor("#FF9F0A")); // สีส้มอินโฟ
                            appendLog(fileName, Color.parseColor("#FFD60A")); // สีเหลืองนีออนตัวไฟล์
                            appendLog("  📍 บรรทัดที่: ", Color.parseColor("#FF9F0A")); 
                            appendLog(lineNumber + "\n", Color.parseColor("#FFD60A")); // สีเหลืองนีออนเลขบรรทัด
                        } else {
                            appendLog("##[error] TARGET  -> ไม่สามารถระบุตำแหน่งไฟล์ในระบบคอมไพล์ได้ชัดเจน\n", Color.parseColor("#FF9F0A"));
                        }

                        appendLog("##[error] DETAIL  -> " + text.trim(), Color.parseColor("#FF8A80")); // สีแดงพาสเทลตัวเนื้อหา Error ดั้งเดิม
                        appendLog("##[endgroup]", Color.parseColor("#FF453A"));

                        // พ่นสรุปสถานะแบบกล่อง ANSI Terminal สวยงาม
                        appendLog("\n┏━━━━━━━━━━━━━━━━━━━━━ Compilation Failure Summary ━━━━━━━━━━━━━━━━━━━━━┓", Color.parseColor("#FF453A"));
                        appendLog("  ❌ ล้มเหลว : การทำงานผิดพลาด (Process completed with exit code 1)", Color.parseColor("#FF7B72"));
                        if (parseSuccess) {
                            appendLog("  🎯 ชี้เป้า   : กรุณาแก้ไขโค้ดที่ไฟล์ " + fileName + " ตรงบรรทัดที่ " + lineNumber, Color.parseColor("#FF9F0A"));
                        }
                        appendLog("  💡 แนะนำ   : โปรดเช็คตัวสะกด, วงเล็บปิด หรือลืมใส่เครื่องหมายเซมิโคลอน (;) ในหน้าจอ Editor", Color.parseColor("#30D158")); // ใช้สีเขียวนีออนตัดเพื่อให้คำแนะนำดูเป็นทางออกที่สดใส
                        appendLog("┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛", Color.parseColor("#FF453A"));
                        
                        showToast("💥 บิวด์ล้มเหลว! (Exit Code 1)");
                        return;
                    }

                    // หากไม่มี Error ให้พ่นเนื้อหาล็อกปกติออกมาตามเฉดสี GitHub Dark Mode
                    if (color == Color.GREEN) {
                        appendLog(text, Color.parseColor("#30D158")); // เขียวนีออน
                    } else if (color == Color.YELLOW) {
                        appendLog(text, Color.parseColor("#FFD60A")); // เหลืองนีออน
                    } else if (color == Color.CYAN) {
                        appendLog(text, Color.parseColor("#64D2FF")); // ฟ้าไอซ์บลู
                    } else {
                        appendLog(text, Color.parseColor("#E5E5EA")); // ขาวเทาอ่านง่ายสบายตา
                    }
                }
