package com.dev.ministudio; // 🟢 แก้ไขเป็น package ตัวพิมพ์เล็กตามหลักไวยากรณ์ Java
...
public class MainActivity extends AppCompatActivity {
    ...
    private CodeEditor codeEditor; // ปรับเปลี่ยนจาก EditText -> CodeEditor ของ Sora
    private DrawerLayout drawerLayout;
    private ListView treeView; 
    private LinearLayout searchBar;
    private android.widget.EditText etFind, etReplace; // ระบุแน่ชัดป้องกันประเภทตีกัน
    ...
    private void initViews() {
        ...
        codeEditor = findViewById(R.id.codeEditor); // ผูกไอดีเข้า Sora Widget
        tvSaveStatus = findViewById(R.id.tvSaveStatus);
        tvFilePath = findViewById(R.id.tvFilePath); 
        
        treeView = findViewById(R.id.treeView); // 🟢 แก้ไข ID ให้ตรงกับ activity_main.xml แล้ว
    ...
    private void setupLogic() {
        ...
        // การตั้งค่าเริ่มต้นขุมพลังขั้นสูงให้แก่ Sora Editor ตัวใหม่
        codeEditor.setEditorLanguage(new JavaLanguage()); // เปิดใช้งานระบบสแกนโค้ด Java แท้
        codeEditor.setColorScheme(new SchemeDarcula()); // 🛠️ [จุดที่ 1 แก้ไข]: เปลี่ยนจาก .INSTANCE เป็นการสร้างออบเจกต์ใหม่ (new)
        codeEditor.setTextSize(currentCodeFontSize); // โหลดขนาดตัวอักษรเริ่มต้น
        codeEditor.setTypefaceText(android.graphics.Typeface.MONOSPACE); // ปรับแต่งฟอนต์รหัสให้กว้างเท่ากันเรียงสวย
        codeEditor.setLineSpacing(2f, 1.2f); // เพิ่มระยะห่างเว้นบรรทัดให้อ่านง่าย
        codeEditor.setWordwrap(false); // ปิดการตัดคำอัตโนมัติ เพื่อให้เลื่อนแถบแนวนอนได้แบบมืออาชีพ
        codeEditor.setUndoEnabled(true); // เปิดระบบจดจำสถานะย้อนกลับ
        
        // 🛠️ [จุดที่ 2 แก้ไข]: คอมเมนต์ปิดใช้งานชั่วคราว เนื่องจากเวอร์ชันนี้ไม่มีเมธอด setOverScrollPage เพื่อให้ระบบบิวด์ผ่านได้
        // codeEditor.setOverScrollPage(true); 
        
        codeEditor.setHighlightCurrentBlock(true); // ไฮไลท์จับคู่ปีกกาเข้าชุดกันอัตโนมัติ

        // เปลี่ยนระบบดักฟังข้อความ TextWatcher เดิมมาเป็นระบบ Event Subscriber ที่เสถียรและเร็วของ Sora
    ...
    private FileNode findParentNode(FileNode childNode) {
        ...
        for (int i = lastClickedPosition; i >= 0; i--) {
            ...
            if (potentialParent.isDirectory && potentialParent.depth < childNode.depth) {
                return potentialParent; // 🟢 แก้ไขจาก parentNode เป็น potentialParent แล้ว
            }
        }
        return null;
    }
    ...
    private void setupShortcutBar(LinearLayout bar) {
        ...
        android.graphics.drawable.GradientDrawable zoomBtnBg = new android.graphics.drawable.GradientDrawable();
        zoomBtnBg.setColor(android.graphics.Color.parseColor("#D32F2F")); 
        zoomBtnBg.setCornerRadius(dpToPx(6));
        ...
        for (final String s : symbols) {
            ...
            if (s.equals("Enter")) {
                ...
                btn.setPadding(dpToPx(18), 0, dpToPx(18), 0);
                btn.setBackground(zoomBtnBg.getConstantState().newDrawable());
                btn.setTextColor(android.graphics.Color.WHITE);
                btn.setTextSize(16); 
            } else {
            ...
            btn.setOnClickListener(v -> {
                if (s.equals(" - ")) {
                    if (currentCodeFontSize > 10.0f) {
                        currentCodeFontSize -= 1.5f;
                        applyEditorFontSize(currentCodeFontSize);
                    }
                } else if (s.equals(" + ")) {
                    if (currentCodeFontSize < 23.0f) {
                        currentCodeFontSize += 1.5f;
                        applyEditorFontSize(currentCodeFontSize);
                    }
                } else {
                    if (codeEditor != null) {
                        // 🛠️ [จุดที่ 3, 4, 5 แก้ไข]: ดึงตำแหน่งแถวและคอลัมน์ของเคอร์เซอร์ออกมา แล้วแทรกข้อความผ่านออบเจกต์เนื้อหา (.getText().insert)
                        int line = codeEditor.getCursor().getLeftLine();
                        int column = codeEditor.getCursor().getLeftColumn();
                        if (s.equals("Enter")) {
                            codeEditor.getText().insert(line, column, "\n");
                        } else if (s.equals("Tab")) {
                            codeEditor.getText().insert(line, column, "    "); 
                        } else {
                            codeEditor.getText().insert(line, column, s);
                        }
                    }
                }
            });
    ...
    private void soraSelectLinear(int startIdx, int endIdx) {
        try {
            ...
            runOnUiThread(() -> {
                // 🟢 แก้ไขเสร็จสมบูรณ์: เรียกใช้งาน setSelectionRegion ผ่านวัตถุเอดิเตอร์หลักโดยตรง
