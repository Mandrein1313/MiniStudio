...
public class MainActivity extends AppCompatActivity {
    ...
    // 🌟 เมทอดรันบิวด์บนคลาวด์เวอร์ชันอัปเกรด: ละเอียดขั้นสุด + ตรวจเจอ Error แล้วสั่งหยุดตัดวงจรทันที
    private void startCloudBuildPipeline() {
        ...
        if (tvConsoleLog != null) {
            tvConsoleLog.setText(""); // ล้างหน้าจอ Log เก่าก่อนเริ่มรันใหม่
        }

        // ตัวแปรสถานะภายใน Listener เพื่อเช็คว่าพังกลางคันหรือไม่
        final boolean[] isAbortedDueToError = {false};

        appendLog("🛠️ [1/3] เริ่มจัดเตรียมโครงสร้างไฟล์และบันทึกโค้ดปัจจุบัน...", Color.CYAN);
        appendLog("📂 Path: " + currentProject.getRootPath(), Color.GRAY);
        ...
        BuildTaskManager buildTask = new BuildTaskManager(
            ...
            new BuildTaskManager.BuildListener() {
                ...
                public void onLogAppend(String text, int color) { 
                    // 🛑 ตรวจสอบก่อน: ถ้าถูกสั่งสกัดจับตัดวงจรไปแล้ว ไม่ต้องพ่น Log อื่นๆ ที่ตามมาอีก
                    if (isAbortedDueToError[0]) return;

                    // แปลงข้อความเป็นตัวพิมพ์เล็กเพื่อใช้ดักจับ Keyword บั๊ก
                    String lowerText = text.toLowerCase();

                    // 🚨 กฎเหล็กสกัดจับ: ถ้าเจอคำว่า error:, compilation failed, หรือ exception
                    if (lowerText.contains("error:") || 
                        lowerText.contains("failed") || 
                        lowerText.contains("exception") || 
                        color == Color.RED) {
                        
                        isAbortedDueToError[0] = true; // เปิดสวิตช์สั่งหยุดทำงาน!
                        
                        // พ่นแจ้งเตือนเหตุการณ์พังแบบละเอียดระยิบ
                        appendLog("\n🛑 [CRITICAL ERROR DETECTED] พบบั๊กในซอร์สโค้ด!", Color.parseColor("#FF5252"));
                        appendLog("📝 บรรทัดปัญหาชี้เป้า: " + text.trim(), Color.parseColor("#FF8A80"));
                        appendLog("\n---------------------------------------------------------", Color.RED);
                        appendLog("🚨 [BUILD ABORTED] ระบบสั่งหยุดคอมไพล์ทันที เพื่อป้องกันความเสียหาย", Color.RED);
                        appendLog("💡 คำแนะนำ: ตรวจสอบวงเล็บ, เซมิโคลอน (;), หรือการ Import คลาสในหน้าตัวแก้ไข", Color.parseColor("#FFB74D"));
                        appendLog("---------------------------------------------------------\n", Color.RED);
                        
                        showToast("💥 บิวด์ล้มเหลว! พบ Error และสั่งหยุดระบบแล้ว");
                        return;
                    }

                    // ถ้าผ่านการตรวจสอบ (ไม่มี Error) ให้พ่น Log ทำงานตามปกติ
                    appendLog(text, color); 
                }
                
                @Override 
                public void onBuildStarted() { 
                    showToast("กำลังส่งซอร์สโค้ดขึ้นรันบนระบบคลาวด์... 🚀"); 
                    appendLog("\n🚀 [2/3] กำลังเชื่อมต่อ Cloud Build Environment และคอมไพล์โปรเจกต์...", Color.YELLOW);
                }

                @Override
                public void onBuildFinished(boolean success, String apkPath) {
                    // ถ้าถูกตัดวงจรไปก่อนหน้านี้แล้ว ไม่ต้องแสดงข้อความสำเร็จหรือข้อความซ้ำซ้อน
                    if (isAbortedDueToError[0]) return;

                    if (success) {
                        showToast("รันโปรเจกต์สำเร็จ! 🎉");
                        appendLog("\n🎉 [3/3] BUILD SUCCESSFUL เสร็จสมบูรณ์!", Color.GREEN);
                        appendLog("📦 Output APK: " + (apkPath != null ? apkPath : "สร้างเสร็จแล้ว"), Color.parseColor("#4CAF50"));
                    } else {
                        showToast("การรันล้มเหลว กรุณาตรวจสอบ Log");
                        appendLog("\n-------------------------------------------", Color.RED);
                        appendLog("🚨 [BUILD FAILED] กระบวนการบิวด์สิ้นสุดลงด้วยความล้มเหลว", Color.RED);
                        appendLog("-------------------------------------------\n", Color.RED);
                    }
                }
            }
