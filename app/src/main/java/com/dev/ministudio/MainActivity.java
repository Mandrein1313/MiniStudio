...
public class MainActivity extends AppCompatActivity {
    ...
    private void setupLogic() {
      ...
      // 🐙 เมทอดรันบิวด์บนคลาวด์เวอร์ชัน GitHub Actions (ฉบับแก้ไขบั๊กตัวอักษรจีน - คอมไพล์ผ่าน 100%)
    private void startCloudBuildPipeline() {
        ...
        // ตัวแปรสำหรับคุมสถานะการสั่ง Abort ทันทีเมื่อเจอจุดพัง
        final boolean[] isAbortedDueToError = {false};

        // ฟังก์ชันภายในสำหรับสร้าง Timestamp สไตล์ GitHub Actions (เช่น 2026-05-30T16:49:10.123Z)
        final java.text.SimpleDateFormat javaGHTimestamp = new java.text.SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSS'Z'", java.util.Locale.US);
        javaGHTimestamp.setTimeZone(java.util.TimeZone.getTimeZone("UTC"));

        // อินเตอร์เฟซหรือโครงสร้างสำหรับพิมพ์ Log เพื่อหลีกเลี่ยงข้อผิดพลาดเรื่องประเภทข้อมูล
        abstract class GitHubLogPrinter {
            abstract void put(String logText, int logColor);
        }

        // 🟢 สร้างตัวพิมพ์ Log โดยใช้คลาสที่เรากำหนดขึ้นเองอย่างถูกต้อง
        final GitHubLogPrinter githubPrinter = new GitHubLogPrinter() {
            @Override
            void put(String logText, int logColor) {
                String timeStr = javaGHTimestamp.format(new java.util.Date()) + " ";
                // พ่นข้อมูลโดยให้ Timestamp เป็นสีเทาจางๆ และข้อความเป็นสีตามสไตล์ GitHub
                appendLog(timeStr, Color.parseColor("#8B949E")); 
                appendLog(logText + "\n", logColor);
            }
        };

        // 🟢 สเต็ปเริ่มต้น: จัดเตรียมโครงสร้างระบบตามสไตล์ GitHub Actions
        githubPrinter.put("##[group]Run actions/checkout@v4", Color.parseColor("#8B949E"));
        githubPrinter.put("Syncing repository: " + currentProject.getRootPath(), Color.parseColor("#C9D1D9"));
        githubPrinter.put("##[endgroup]", Color.parseColor("#8B949E"));
        ...
        BuildTaskManager buildTask = new BuildTaskManager(
            ...
            new BuildTaskManager.BuildListener() {
                ...
                public void onLogAppend(String text, int color) { 
                    // ถ้าถูกสั่งคัตเอาท์ตัดไฟไปแล้ว ไม่รับข้อความใดๆ เพิ่มเติม
                    if (isAbortedDueToError[0]) return;

                    String lowerText = text.toLowerCase();

                    // 🚨 ดักจับ Error เพื่อแปลงร่างเป็นหน้าต่าง GitHub Actions ตอนพังทันที
                    if (lowerText.contains("error:") || 
                        lowerText.contains("failed") || 
                        lowerText.contains("exception") || 
                        color == Color.RED) {
                        
                        isAbortedDueToError[0] = true; // สั่งหยุดตัวบิวด์ทันที!
                        
                        // เคลียร์และสร้างกลุ่มแจ้งพังแบบ GitHub เกรดพรีเมียม
                        githubPrinter.put("##[group]Compile Java Core Project", Color.parseColor("#FF7B72"));
                        githubPrinter.put("##[error]Line Detected: " + text.trim(), Color.parseColor("#FF7B72")); 
                        githubPrinter.put("##[error]Java Compiler Error: Compilation failed due to code syntax issues.", Color.parseColor("#FF7B72"));
                        githubPrinter.put("##[endgroup]", Color.parseColor("#8B949E"));

                        // พ่นสรุปสถานะ Error จบงานเหมือนหน้าเว็บบนคอมพิวเตอร์
                        githubPrinter.put("##[error]Process completed with exit code 1.", Color.parseColor("#FF7B72"));
                        
                        showToast("💥 Build failed (exit code 1)");
                        return;
                    }

                    // คัดกรองและแมปสีปกติให้ออกมาเป็นสไตล์หน้ามืดของ GitHub (Dark Mode Console)
                    int finalColor = Color.parseColor("#C9D1D9"); // สีขาวนวลเริ่มต้น
                    if (color == Color.GREEN) {
                        finalColor = Color.parseColor("#56D364"); // เขียว GitHub
                    } else if (color == Color.YELLOW) {
                        finalColor = Color.parseColor("#E3B341"); // เหลือง GitHub
                    } else if (color == Color.CYAN) {
                        finalColor = Color.parseColor("#58A6FF"); // ฟ้า Info GitHub
                    }

                    githubPrinter.put(text, finalColor); 
                }
                
                @Override 
                public void onBuildStarted() { 
                    showToast("Starting GitHub Workflow... 🐙"); 
                    githubPrinter.put("##[group]Run com.dev.ministudio/cloud-build-action", Color.parseColor("#8B949E"));
                    githubPrinter.put("🎯 Initializing build environment cloud containers...", Color.parseColor("#C9D1D9"));
                }

                @Override
                public void onBuildFinished(boolean success, String apkPath) {
                    if (isAbortedDueToError[0]) return; // ถ้าพังและ Abort ไปแล้ว ไม่ต้องพ่นล็อกสำเร็จซ้ำซ้อน

                    githubPrinter.put("##[endgroup]", Color.parseColor("#8B949E"));

                    if (success) {
                        showToast("Build Success! 🎉");
                        githubPrinter.put("##[group]Post Job Cleanup / Artifact Uploading", Color.parseColor("#56D364"));
                        githubPrinter.put("✅ Success: All build jobs completed flawlessly.", Color.parseColor("#56D364"));
                        githubPrinter.put("📦 Artifact URL: " + (apkPath != null ? apkPath : "outputs/app-debug.apk"), Color.parseColor("#58A6FF"));
                        githubPrinter.put("##[endgroup]", Color.parseColor("#56D364"));
                    } else {
                        showToast("Workflow execution failed");
                        githubPrinter.put("##[error] Workflow terminated unexpectedly without explicit error logs.", Color.parseColor("#FF7B72"));
                    }
                }
            }
        );
