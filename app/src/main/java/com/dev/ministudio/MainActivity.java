...
public class MainActivity extends AppCompatActivity {
    ...
    // Views (จัดกลุ่มให้เป็นระเบียบ ไม่ประกาศซ้ำ)
    ...
    private void replaceText() {
        String query = etFind.getText().toString();
        String replaceStr = etReplace.getText().toString();
        if (query.isEmpty()) return;
        ...
        if (start != end) {
            String selectedText = codeEditor.getText().toString().substring(start, end);
            if (selectedText.equals(query)) {
                codeEditor.getText().replace(start, end, replaceStr);
                lastSearchIndex = start + replaceStr.length();
                findAndHighlight(); 
                return;
            }
        }
    ...
    private void triggerTreeRefresh(FileNode node) {
        // เมธอดเสริมสำหรับสั่ง Refresh ลิสต์กิ่งไม้ตามความต้องการของแอปท่าน
        if (fileTreeAdapter != null) {
            fileTreeAdapter.notifyDataSetChanged();
        }
    }
    ...
    private void openFilePicker() {
        // เมธอดจำลองเพื่อความปลอดภัยของระบบ Import
        showToast("เปิดตัวเลือกไฟล์...");
    }
