...
public class MainActivity extends AppCompatActivity {
    ...
    // 🐙 เมทอดรันบิวด์บนคลาวด์เวอร์ชันอัปเกรดสูงสุด: จัดฟอร์แมตและแสดง Error เป๊ะตามสไตล์ GitHub Actions
    private void startCloudBuildPipeline() {
        ...
        appendLog("##[group]Run Setup & Pre-build Validation", Color.parseColor("#8B949E")); // สีเทา GitHub Style
        appendLog("🔔 [Setting up workflow runtime...] Context initialization completed.", Color.parseColor("#C9D1D9"));
        appendLog("📂 Project Root Path: " + currentProject.getRootPath(), Color.parseColor("#58A6FF")); // สีฟ้า GitHub Info
            ...
            new BuildTaskManager.BuildListener() {
                ...
                public void onLogAppend(String text, int color) { 
                    ...
                    if (lowerText.contains("error:") || 
                        ...
                        // 🐙 ฟอร์แมตแต่งข้อความ Error ให้เหมือนหลุดมาจาก GitHub Actions ใน Terminal เป๊ะๆ
                        appendLog("\n##[group]❌ GitHub Cloud Builder / Compile Failure Details", Color.parseColor("#FF7B72")); 
                        appendLog("##[error] Process completed with exit code 1. (Compilation error detected)", Color.parseColor("#FF7B72")); // สีแดง GitHub Error
                        appendLog("##[error] Location: " + text.trim(), Color.parseColor("#FFA198")); // ไฮไลท์บรรทัดที่พัง
                        appendLog("##[endgroup]", Color.parseColor("#FF7B72"));

                        // พ่นสรุปสถานะ Workflow แบบภาพรวมท้ายสุดของหน้าจอ
                        appendLog("\n------------------------------------------------------------------------", Color.parseColor("#8B949E"));
                        appendLog("❌ Error: Process completed with exit code 1.", Color.parseColor("#FF7B72"));
                        appendLog("💡 Suggestion: Please check your syntax, imports, or missing semicolons (;) in the editor.", Color.parseColor("#F2CC60")); // สีเหลืองทองเตือนสติ
                    ...
                    // หากไม่มี Error ให้พ่นเนื้อหาล็อกปกติออกมา (เปลี่ยนใช้คู่สีโทนมืดของ GitHub เพื่อความสมจริง)
                ...
                public void onBuildStarted() { 
                    showToast("Starting cloud workflow... 🐙"); 
                    appendLog("\n##[group]🚀 Run com.dev.ministudio:compileJava", Color.parseColor("#8B949E"));
                    appendLog("🔄 Connecting to build container environment...", Color.parseColor("#C9D1D9"));
                }
                ...
                public void onBuildFinished(boolean success, String apkPath) {
                    ...
                    if (success) {
                        ...
                        appendLog("\n##[group]🎉 Post-build Task: Artifact Archiving", Color.parseColor("#56D364"));
                        appendLog("✅ SUCCESS: Workflow finished completely without errors.", Color.parseColor("#56D364"));
                        appendLog("📦 Uploading Artifact (APK): " + (apkPath != null ? apkPath : "outputs/apk/debug/app-debug.apk"), Color.parseColor("#58A6FF"));
                        appendLog("##[endgroup]", Color.parseColor("#56D364"));
                    } else {
                        showToast("Workflow failed");
                        appendLog("\n##[error] Run failed due to unexpected environment termination.", Color.parseColor("#FF7B72"));
                    }
                }
            }
        );
