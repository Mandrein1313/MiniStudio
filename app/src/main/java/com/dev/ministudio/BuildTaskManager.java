...
public class BuildTaskManager {
        ...
        new Thread(() -> {
            ...
            try {
                ...
                uiHandler.post(() -> {
                    ...
                    startAutoCheckAndDownload(repoName, username, token);
                });
    ...
    private void startAutoCheckAndDownload(final String repoName, final String username, final String token) {
        ...
        checkRunnable[0] = new Runnable() {
            ...
            public void run() {
                ...
                // 🌟 ตรวจสอบสถานะความล้มเหลวจาก GitHub Actions ก่อนส่งให้ตัวดาวน์โหลดงาน
                new Thread(() -> {
                    checkWorkflowFailure(username, token, repoName);
                }).start();
    ...
    // 🌟 ระบบสืบค้นข้อผิดพลาดแบบสดๆ จากระบบ GitHub Actions API
    private void checkWorkflowFailure(String username, String token, String repoName) {
        try {
            URL url = new URL("https://api.github.com/repos/" + username + "/" + repoName + "/actions/runs?per_page=1");
            HttpURLConnection conn = (HttpURLConnection) url.openConnection();
            conn.setRequestProperty("Authorization", "Bearer " + token);
            conn.setRequestProperty("Accept", "application/vnd.github+json");
            ...
            if (conn.getResponseCode() == 200) {
                ...
                JSONArray workflowRuns = response.getJSONArray("workflow_runs");
                if (workflowRuns.length() > 0) {
                    JSONObject latestRun = workflowRuns.getJSONObject(0);
                    String conclusion = latestRun.optString("conclusion", "null");

                    // 💥 ตรวจสอบหากพบว่าสเตตัสแจ้งพัง (failure) สั่งดึงรายละเอียดทันที
                    if ("failure".equals(conclusion)) {
                        String jobsUrl = latestRun.getString("jobs_url");
                        fetchFailedJobDetails(token, jobsUrl);
                    }
                }
            }
        } catch (Exception e) {
    ...
    // 🌟 ดึงข้อมูลเจาะลึกว่าขั้นตอนย่อยส่วนไหนของ Gradle ที่พัง
    private void fetchFailedJobDetails(String token, String jobsUrl) {
        try {
            URL url = new URL(jobsUrl);
            HttpURLConnection conn = (HttpURLConnection) url.openConnection();
            conn.setRequestProperty("Authorization", "Bearer " + token);
            conn.setRequestProperty("Accept", "application/vnd.github+json");
            ...
            if (conn.getResponseCode() == 200) {
                ...
                for (int i = 0; i < jobs.length(); i++) {
                    ...
                    for (int j = 0; j < steps.length(); j++) {
                        ...
                        if ("failure".equals(step.optString("conclusion", ""))) {
                            
                            // พ่นส่วนหัวแสดงข้อผิดพลาดหลักสีแดงสดบน Console
                            sendProgress("\n🚨 [GRADLE BUILD FAILED AT CLOUD]", COLOR_ERROR);
                            sendProgress("💥 ขั้นตอนที่พัง: " + step.getString("name"), COLOR_ERROR);
                            sendProgress("📝 กรุณาตรวจเช็ค Syntax โค้ด, ปีกกาปิด หรือการ Import คลาสในแอปให้ถูกต้อง\n", COLOR_WARNING);
                            return;
                        }
                    }
                }
